...
class WodSpec extends FreeSpec with Matchers {
  ...
  "For A WOD" - {
    "The description is parsed and stored in a print friendly format" in {
      ...
      val wod = Wod("date", wodDescription, "link")

      wod.description shouldBe
        """SPEED 4 Rounds for Time
          | 25 Calorie Row 10 Burpees over the rower
          |
          | WOD
          | Death by&#8230; Wall Balls 20/14 Pull Ups
          |
          | Alternate movements
          | Min 1
          | 1 Wall Ball Min 2
          | 2 Pull Ups Min 3
          | 3 Wall Balls&#8230; etc etc""".stripMargin
    }
  }
}
